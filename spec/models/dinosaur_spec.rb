...
RSpec.describe Dinosaur, type: :model do
  ...
  context "scopes" do
    let!(:caged_carnivore) { create(:dinosaur, :carnivore, cage:) }
    let(:not_caged_herbivore) { create(:dinosaur, :herbivore) }
    let!(:cage) { create(:cage, :active) }
    it "#caged, #not_caged" do
      expect(described_class.not_caged).to match_array([not_caged_herbivore])
      expect(described_class.caged).to match_array([caged_carnivore])
    end
    it "#by_species_name" do
      expect(described_class.by_species_name(caged_carnivore.species.to_s)).to match_array([caged_carnivore])
    end
    it "#herbivore" do
      expect(described_class.herbivore).to match_array([not_caged_herbivore])
    end
    it "#carnivore" do
      expect(described_class.carnivore).to match_array([caged_carnivore])
    end
  end
